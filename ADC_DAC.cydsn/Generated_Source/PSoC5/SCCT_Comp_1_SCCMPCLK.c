/*******************************************************************************
* File Name: SCCT_Comp_1_SCCMPCLK.c
* Version 2.20
*
*  Description:
...
#include "SCCT_Comp_1_SCCMPCLK.h"
...
/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_Start
********************************************************************************
*
* Summary:
...
void SCCT_Comp_1_SCCMPCLK_Start(void) 
{
    ...
    SCCT_Comp_1_SCCMPCLK_CLKEN |= SCCT_Comp_1_SCCMPCLK_CLKEN_MASK;
	SCCT_Comp_1_SCCMPCLK_CLKSTBY |= SCCT_Comp_1_SCCMPCLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_Stop
********************************************************************************
*
* Summary:
...
void SCCT_Comp_1_SCCMPCLK_Stop(void) 
{
    ...
    SCCT_Comp_1_SCCMPCLK_CLKEN &= (uint8)(~SCCT_Comp_1_SCCMPCLK_CLKEN_MASK);
	SCCT_Comp_1_SCCMPCLK_CLKSTBY &= (uint8)(~SCCT_Comp_1_SCCMPCLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SCCT_Comp_1_SCCMPCLK_CLKEN & SCCT_Comp_1_SCCMPCLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SCCT_Comp_1_SCCMPCLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SCCT_Comp_1_SCCMPCLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SCCT_Comp_1_SCCMPCLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SCCT_Comp_1_SCCMPCLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SCCT_Comp_1_SCCMPCLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SCCT_Comp_1_SCCMPCLK_GetSourceRegister();
    uint16 oldDivider = SCCT_Comp_1_SCCMPCLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SCCT_Comp_1_SCCMPCLK_CLKEN & SCCT_Comp_1_SCCMPCLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SCCT_Comp_1_SCCMPCLK_DIV_PTR, clkDivider);
                SCCT_Comp_1_SCCMPCLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SCCT_Comp_1_SCCMPCLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SCCT_Comp_1_SCCMPCLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SCCT_Comp_1_SCCMPCLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SCCT_Comp_1_SCCMPCLK_DIV_PTR, clkDivider);
				SCCT_Comp_1_SCCMPCLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SCCT_Comp_1_SCCMPCLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SCCT_Comp_1_SCCMPCLK_MOD_SRC |= modeBitMask & (uint8)SCCT_Comp_1_SCCMPCLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SCCT_Comp_1_SCCMPCLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SCCT_Comp_1_SCCMPCLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SCCT_Comp_1_SCCMPCLK_MOD_SRC & (uint8)(SCCT_Comp_1_SCCMPCLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SCCT_Comp_1_SCCMPCLK_GetDividerRegister();
    uint8 oldSrc = SCCT_Comp_1_SCCMPCLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SCCT_Comp_1_SCCMPCLK_MOD_SRC |= CYCLK_SSS;
        SCCT_Comp_1_SCCMPCLK_MOD_SRC =
            (SCCT_Comp_1_SCCMPCLK_MOD_SRC & (uint8)(~SCCT_Comp_1_SCCMPCLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SCCT_Comp_1_SCCMPCLK_MOD_SRC & SCCT_Comp_1_SCCMPCLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SCCT_Comp_1_SCCMPCLK_PHASE = clkPhase & SCCT_Comp_1_SCCMPCLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SCCT_Comp_1_SCCMPCLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SCCT_Comp_1_SCCMPCLK_PHASE & SCCT_Comp_1_SCCMPCLK_PHASE_MASK;
}
