...
/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_Start
********************************************************************************
*
* Summary:
...
void ADC_CLOCK_SOURCE_Start(void) 
{
    ...
    ADC_CLOCK_SOURCE_CLKEN |= ADC_CLOCK_SOURCE_CLKEN_MASK;
	ADC_CLOCK_SOURCE_CLKSTBY |= ADC_CLOCK_SOURCE_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_Stop
********************************************************************************
*
* Summary:
...
void ADC_CLOCK_SOURCE_Stop(void) 
{
    ...
    ADC_CLOCK_SOURCE_CLKEN &= (uint8)(~ADC_CLOCK_SOURCE_CLKEN_MASK);
	ADC_CLOCK_SOURCE_CLKSTBY &= (uint8)(~ADC_CLOCK_SOURCE_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ADC_CLOCK_SOURCE_CLKEN & ADC_CLOCK_SOURCE_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ADC_CLOCK_SOURCE_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC_CLOCK_SOURCE_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADC_CLOCK_SOURCE__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC_CLOCK_SOURCE_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_CLOCK_SOURCE_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ADC_CLOCK_SOURCE_GetSourceRegister();
    uint16 oldDivider = ADC_CLOCK_SOURCE_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_CLOCK_SOURCE_CLKEN & ADC_CLOCK_SOURCE_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADC_CLOCK_SOURCE_DIV_PTR, clkDivider);
                ADC_CLOCK_SOURCE_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADC_CLOCK_SOURCE_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADC_CLOCK_SOURCE_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_CLOCK_SOURCE_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ADC_CLOCK_SOURCE_DIV_PTR, clkDivider);
				ADC_CLOCK_SOURCE_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ADC_CLOCK_SOURCE_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_CLOCK_SOURCE_MOD_SRC |= modeBitMask & (uint8)ADC_CLOCK_SOURCE_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_CLOCK_SOURCE_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_CLOCK_SOURCE_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_CLOCK_SOURCE_MOD_SRC & (uint8)(ADC_CLOCK_SOURCE_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ADC_CLOCK_SOURCE_GetDividerRegister();
    uint8 oldSrc = ADC_CLOCK_SOURCE_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_CLOCK_SOURCE_MOD_SRC |= CYCLK_SSS;
        ADC_CLOCK_SOURCE_MOD_SRC =
            (ADC_CLOCK_SOURCE_MOD_SRC & (uint8)(~ADC_CLOCK_SOURCE_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_CLOCK_SOURCE_MOD_SRC & ADC_CLOCK_SOURCE_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ADC_CLOCK_SOURCE_PHASE = clkPhase & ADC_CLOCK_SOURCE_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_CLOCK_SOURCE_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ADC_CLOCK_SOURCE_PHASE & ADC_CLOCK_SOURCE_PHASE_MASK;
}
